...
public class Test02 {
	...
	public int calc(int no1, int no2) {
		// 매개변수는 여러개가 입력 될 경우 , 단위로 나열한다..
		
		// 반환값 변수
		int result = 0;
		
		// 이 함수를 실행할 때 입력된 두 정수는 no1, no2에 기억될 것이다..
		// 이 변수에 기억된 데이터를 계산하면 된다..
		
		result = no1 + no2;
		
		// 반환값 내보내기
		return result;		// 1. 실행중인 이 함수를 즉시 종료하세요.. / 2. 결과값이 있으면 결과값을 내보내고 함수를 종료하세요..
	...
	public void toPrint(int no1, int no2) {
		...
		return;		// 이 명령을 기술하지 않아도 실행내용을 모두 실행했으므로 이 함수가 자동 종료된다..
					// 따라서 생략해도 괜찮다..
	}
	
	
	
	// 두 수 와 결과를 입력해주면 계산결과를 출력해주는 함수
