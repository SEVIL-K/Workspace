...
public class Synch02 {
    ...
    synchronized void makeData(int num) {
        if (list.size() == 5) {
            // 이 경우는 데이터가 5개 채워져 있으므로 더이상 작업을 하면 안된다.

            try {
                System.out.println("채우는 작업 완료");
                wait();
                // 이 순간 이 함수를 사용하는 스레드는 block 상태로 전위가 된다.
            } catch (Exception e) {
                e.printStackTrace();
            }
        }
        ...
        // 리스트에 숫자 하나를 채운다.
        list.add(num);
        System.out.println(num + " 를 채웠습니다. \t\t ** 채운 후 데이터의 갯수는 " + list.size() + " 개 입니다.");
    ...
    synchronized void useData() {
        if (list.size() == 0) {
            // 이 경우는 데이터를 꺼내서 사용할 수 없으므로 block 상태로 전위가 되어야 한다.
        ...
        int no = list.get(0);
        // 꺼낸 데이터는 리스트에서 제거한다.
        list.remove(0);     // 꺼낸 데이터를 제거했으므로 입력할 수 있는 공간이 하나 생겼다.

        System.out.println("\t\t" + no + " 를 꺼냈습니다. \t\t ** 꺼낸 후 남아있는 데이터의 갯수는 " + list.size() + " 개 입니다.");

        // 이제 데이터를 추가할 수 있는 공간이 생겼으니 다시 데이터를 채우는 작업을 Runnable 상태로 전위시킨다.
        notify();
...
class DataThread01 extends Thread {
    ...
    public void run() {
        ...
        for (int i = 0; i < 50; i++) {
            int no = (int) (Math.random() * 99 + 1);    // 1 ~99
            main.makeData(no);
