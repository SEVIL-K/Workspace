...
public class PstmtTest01 {
    ...
    private ResultSet rs;                   // 여기에 선언된 변수들은 멤버변수로 자동으로 null 로 초기화.
    ...
    public PstmtTest01() {
        ...
        try {
            ...
            // 4. 명령을 전달할 도구 준비
            /*
                Statement ==> 프로그램에서 JDBC 로 명령을 전달하는 도구
                                이 때 사용해야되는 질의명령은 완벽한 질의명령이어야 한다.

                우리의 경우는 매번 부서번호를 정해서 바꿔야하는 불완전한 질의명령이다.
                이럴 때 사용하는 명령전달도구로 PreparedStatement 가 있다.
             */

            pstmt = con.prepareStatement(sql);

            /*
                명령전달도구는 준비되어 있지만 도구안에 세팅된 질의명령은 아직 완벽하지 않다.
                따라서 질의명령을 완벽하게 만들어주어야 한다.
                완벽한 질의명령으로 만드는 방법은 ? 에 정확한 데이터를 채워주면 된다.

                불완전한 질의명령 ( ? 있는 질의명령 ) 에 데이터를 채우는 방법
                    문자열인 경우
                    ==> 변수.setString(위치값, 데이터);

                    정수(int)인 경우
                    ==> 변수.setInt(위치값, 데이터);
                    ...

                    이 때 위치값은 문자열 중에서 ? 의 순서를 의미하고 시작값은 1부터 시작한다.
             */

            pstmt.setInt(1, dno);

            // 질의명령이 완성되었으므로 JDBC 에게 전달하고 결과를 받는다.
            rs = pstmt.executeQuery();

            while (rs.next()) {     // rs.next() ==> 작업실행줄을 한 줄 내려주는 함수. 내린결과 꺼내올 데이터가 있으면 true 반환.
