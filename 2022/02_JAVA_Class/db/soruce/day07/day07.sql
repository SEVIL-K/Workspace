...
-- 아바타 테이블의 제약조건 조회
SELECT constraint_name 제약조건이름,
       constraint_type 제약조건,
       table_name      테이블이름
FROM user_constraints
WHERE table_name IN ('AVATAR', 'MEMBER')
;
...
SELECT e1.EMPNO 사원번호, e1.ENAME 사원이름, e1.SAL 급여, (m.msal-e1.SAL) 부서최대급여와의차이, m.msal 부서최대급여, m.asal 부서평균급여
FROM (SELECT e.EMPNO, max(SAL) msal, round(avg(SAL)) asal
FROM EMP e,
     DEPT d
WHERE e.DEPTNO = d.DEPTNO
GROUP BY d.DEPTNO) m, EMP e1
